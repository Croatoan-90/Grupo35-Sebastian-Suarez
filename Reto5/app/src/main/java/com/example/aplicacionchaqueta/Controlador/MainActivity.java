...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if(id == R.id.menu1){
            ...
            change.replace(R.id.containerFragment,prod).addToBackStack(null).commit();

        }
        if(id == R.id.menu2){
            change = getSupportFragmentManager().beginTransaction();
            change.replace(R.id.containerFragment,serv).addToBackStack(null).commit();

        }
        if(id == R.id.menu3){
            change = getSupportFragmentManager().beginTransaction();
            change.replace(R.id.containerFragment,suc).addToBackStack(null).commit();

        }
        if(id == R.id.iconB){
            change = getSupportFragmentManager().beginTransaction();
            change.replace(R.id.containerFragment,mainScreen).addToBackStack(null).commit();

        }
        if(id == R.id.icon2_B){
            change = getSupportFragmentManager().beginTransaction();
            change.replace(R.id.containerFragment,fav).addToBackStack(null).commit();

        }
        if(id == R.id.icon3_B){
            change = getSupportFragmentManager().beginTransaction();
            change.replace(R.id.containerFragment,shoppingCar).addToBackStack(null).commit();
